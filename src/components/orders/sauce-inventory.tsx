...
export function SauceInventory() {
  ...
  const inventory = FLAVORS.map(f => {
    ...
    return { flavor: f, packed: p, drumBottles: dw, total: p + dw }
  })

  const totalBottles = inventory.reduce((s, i) => s + i.total, 0)

  const numCell = (val: number, dash?: boolean) => {
    if (dash && val === 0) return <span className="text-muted-foreground/20">&mdash;</span>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            <tr className="border-t border-border/50 text-muted-foreground text-xs">
              <td className="py-1 pr-4">&rarr; Cases (&divide;6)</td>
              {FLAVORS.map(f => (
                <td key={f} className="py-1 text-right tabular-nums px-2">
                  {numCell(Math.floor((packed[f] || 0) / 6), true)}
                </td>
              ))}
            </tr>
            <tr className="border-t border-border/50">
            ...
            <tr className="border-t border-border/50 text-muted-foreground text-xs">
              <td className="py-1 pr-4">&rarr; Bottles (&times;{DRUM_BOTTLES})</td>
              {FLAVORS.map(f => {
                const inv = inventory.find(i => i.flavor === f)
                return (
                  <td key={f} className="py-1 text-right tabular-nums px-2">
                    {numCell(inv?.drumBottles || 0, true)}
                  </td>
                )
              })}
            </tr>
              ...
              {FLAVORS.map(f => {
                ...
                return (
                  <td key={f} className="pt-2 text-right tabular-nums px-2">
                    <span className="font-semibold tabular-nums">{(inv?.total || 0).toLocaleString()}</span>
                  </td>
                )
              })}
            </tr>
